...
mark_match = mark_compile.match('comedy comedy')
...
my_match = re.compile('"Who').match(string_dict['victor_string'])
print(f'victor match: {my_match}')

my_search = re.compile('SpongeBob').search(string_dict['victor_string'])
print(my_search)

my_findall = re.compile('SpongeBob').findall(string_dict['victor_string'])
print(my_findall)
print(len(my_findall))

# Match based on Regex bracket syntax. E.g., re.compile('[A-Z][a-z]')

my_pattern = re.compile('[a-z]').search(string_dict['george_string'])
print(my_pattern)

# FIND ANY CHARACTER WITH REGEX, LETTER, DIGIT, ANYTHING
...
emails = ['blue1@gmail.org', 'gr2een@gmail.com', '3red@gmail.com']

email_pattern = re.compile('([\w]+)@([a-z]+)(.com|.org)') # email validator
...
# Scottish names McDonald

names = ['Shannon', 'Caitlin', 'Jason', 'McDonald', 'MacFarland', 'MacDuck']

def scottish_validator(list_a):
    scot_pattern = re.compile('([A-Z][a-z]+[A-Z][a-z])')
    print(scot_pattern)

    for item in list_a:
        if scot_pattern.match(item):
            print('You\'re a scottish lass')
        else:
            print('Get out of here, you Irish porridge')

print(scottish_validator(names))
